...
void getCount(pos A1, pos A2, pos B1, pos B2) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            // x축 먼저
            for (int i = x1; i <= x2; i++) board[y1][i] = -1;
            for (int i = y1; i <= y2; i++) board[i][x2] = -1;
        } else {
            // y축 먼저
            for (int i = y1; i <= y2; i++) board[i][x1] = -1;
            for (int i = x1; i <= x2; i++) board[y2][i] = -1;
        }
        while (!q.empty()) q.pop();
        ...
        while (!q.empty()) {
            pos cur = q.front();
            q.pop();
            board[cur.y][cur.x] = cur.cnt;
            if (cur.x == B2.x && cur.y == B2.y) break;
            for (int i = 0; i < 4; i++) {
                nx = cur.x + dx[i];
                ny = cur.y + dy[i];
